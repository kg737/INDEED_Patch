#' @title Interactive Network Visualization for INDEED Partial and Non-Partial Correlation
...
networkVis <- function(results = NULL, nodesize= 'Node_Degree', nodecolor= 'Activity_Score', edgewidth= 'NO', layout= 'nice'){
  
  nodes <- results$activity_score
  links <- results$diff_network
  vis.nodes <- data.frame(id= nodes$Node, name= nodes$MetID, pval= nodes$P_value, ndegree= nodes$Node_Degree, ascore= nodes$Activity_Score, stringsAsFactors = FALSE)
  ...
  net <- visNetwork(vis.nodes, vis.links, width = "100%", height = "800px", main= "INDEED 2.0", submain= paste("Node size is reprentative of: ", nodesize)) %>%
    ...
    visInteraction( dragView= TRUE, dragNodes= TRUE, zoomView= TRUE, navigationButtons= FALSE, hideEdgesOnDrag= FALSE) %>%
